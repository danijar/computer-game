...
T *ManagerData::Load(uint64_t Id, T* Instance)
{
	...
	if(result == SQLITE_ROW)
		...
		ManagerLog::Warning("data", "no row found");
	else
		ManagerLog::Fail("data", "query from (" + name + ") failed with code (" + to_string(result) + ")");
...
std::unordered_map<uint64_t, T*> ManagerData::Load()
{
	...
	while(int result = sqlite3_step(statement))
	{
		...
		else { ManagerLog::Fail("data", "query from (" + name + ") failed with code (" + to_string(result) + ")"); break; }
	};
