...
sqlite3 *ManagerData::Open(string Path)
{
	...
	if(result != SQLITE_OK)
		ManagerLog::Fail("data", "connect to database fail");
	return database;
}
...
bool ManagerData::Close(sqlite3 *Database)
{
	...
	if(result != SQLITE_OK)
	{
		ManagerLog::Fail("data", "disconnect from database failed");
		return false;
	}
	// delete Connection;
	return true;
...
bool ManagerData::Query(sqlite3 *Database, string Sql, function<void(sqlite3_stmt*)> Callback)
{
	...
	while(int result = sqlite3_step(statement))
	{
		...
		{
			ManagerLog::Fail("data", "query failed with code (" + to_string(result) + ")");
			return false;
		}
	}
	return true;
