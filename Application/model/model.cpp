...
class ModuleModel : public Module
{
	...
	void Listeners()
	{
		Event->Listen("WindowFocusGained", [=]{
			meshes.Reload();
			materials.Reload(); // How to apply reloaded materials? They don't have constant OpenGL ids.
			textures.Reload();
		});
	...
	unsigned int Model(string Mesh, string Material, vec3 Position = vec3(0), vec3 Rotation = vec3(0), vec3 Scale = vec3(1), bool Static = true)
	{
		...
		Meshes::Data mesh = meshes.Get(Mesh);
		frm->Positions = mesh.Positions;
		frm->Normals   = mesh.Normals;
		frm->Texcoords = mesh.Texcoords;
		frm->Elements  = mesh.Elements;

		Materials::Data material = materials.Get(Material);
		frm->Diffuse   = textures.Get(material.Diffuse);
