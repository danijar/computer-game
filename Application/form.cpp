...
class ComponentForm : public Component
{
	...
	void Listeners()
	{
		...
		Event->Listen("WindowRecreated", [=]{
			...
			for(auto i = fms.begin(); i != fms.end(); ++i)
			{
				//glBindBuffer(GL_ARRAY_BUFFER, i->second->VerticesPosition);
				//glBindBuffer(GL_ARRAY_BUFFER, i->second->VerticesColor);
				//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, i->second->Elements);
			}
		});
	}
