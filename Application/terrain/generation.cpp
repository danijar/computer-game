...
void ModuleTerrain::Generate(Terrain *Terrain)
{
	...
	for(int x = 0; x < CHUNK_SIZE.x; ++x)
	{
		for(int z = 0; z < CHUNK_SIZE.z; ++z)
		{
			...
			float densityamount = 1 / (1 + pow(10000, simplex(0.3f / maxground * vec2(offset.x + x, offset.z + z)) + 0.5f));
			...
			for(int y = 0; y < CHUNK_SIZE.y; ++y)
			{
				...
				//densities.push_back(1.0f * (simplex(0.9f / maxground * vec3(offset.x + x, offset.y + y, offset.z + z)) / 1.5f + 0.3f));
				//densities.push_back(0.5f * (simplex(2.0f / maxground * vec3(offset.x + x, offset.y + y, offset.z + z)) / 2));
